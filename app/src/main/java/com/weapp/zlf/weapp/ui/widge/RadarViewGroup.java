...
public class RadarViewGroup extends ViewGroup implements RadarView.IScanningListener {
    ...
    private List<BtMacInfo> mDatas;//数据源
...
//    private int minItemPosition;//最小距离的item所在数据源中的位置
     ...
     * @param mDatas
     */
    public void setDatas(List<BtMacInfo> mDatas) {
        this.mDatas = mDatas;
        dataLength = mDatas.size();
        float min = Float.MAX_VALUE;
        float max = Float.MIN_VALUE;
        //找到距离的最大值，最小值对应的 minItemPosition
        for (int j = 0; j < dataLength; j++) {
//            BtMacInfo item = mDatas.get(j);
...
//            }
            ...
            circleView.setProportion((new Random().nextInt(100) / 100f + 0.6f) * 0.52f);
            if (0 == i) {
...
//        LogUtil.m("完成回调");
    ...
    private void startAnim(CircleView object, int position) {
        if (object != null) {
            object.setPortraitIcon(mResIds[position % mResIds.length]);
