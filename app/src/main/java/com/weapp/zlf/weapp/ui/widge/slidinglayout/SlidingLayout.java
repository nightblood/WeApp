package com.weapp.zlf.weapp.ui.widge.slidinglayout;
...
//                Log.i("onInterceptTouchEvent", "down");
                ...
                Log.d(TAG, "onInterceptTouchEvent: down==> " + initialDownY);
                if (initialDownY == -1) {
                    return false;
                }
...
//                    Log.e(LOG_TAG, "Got ACTION_MOVE event but don't have an active pointer id.");
                ...
                Log.d(TAG, "onInterceptTouchEvent: move==> " + y);
                if(y > mInitialDownY) {
                    //判断是否是上拉操作
                    final float yDiff = y - mInitialDownY;
                    Log.d(TAG, "onInterceptTouchEvent: 上拉==> " + yDiff);
                    ...
                    Log.d(TAG, "onInterceptTouchEvent: 下拉==> " + yDiff);
...
//                Log.i("onInterceptTouchEvent", "up");
    ...
    public boolean onTouchEvent(MotionEvent event) {
        if(mDelegateTouchListener != null && mDelegateTouchListener.onTouch(this,event) || !mIsBeingDragged){
            return true;
        }
        switch (event.getAction()){
            case MotionEvent.ACTION_DOWN:
                Log.d(TAG, "onTouchEvent: down==> " + getMotionEventY(event, mActivePointerId));
                break;
            case MotionEvent.ACTION_MOVE:
                float delta = 0.0f;
                ...
                if(mSlidingPointerMode == SLIDING_POINTER_MODE_MORE) {
                    ...
                    if (mActivePointerId != activePointerId) {
                        //change pointer
                        Log.d(TAG, "onTouchEvent: change pointer");
...
//                            / mSlidingOffset;
                    ...
                    //滑动阻力计算
//                    float tempOffset = getInstrument().getTranslationY(mTargetView) + delta;

                    float tempOffset = 1 - (Math.abs(getInstrument().getTranslationY(mTargetView) + delta) / mTargetView.getMeasuredHeight());

                    Log.d(TAG, "onTouchEvent: sliding offset==> " + mSlidingOffset + ", temp offset==> " + tempOffset +
                            ", translationY==> " +getInstrument().getTranslationY(mTargetView) + ", last motion Y==> " + mLastMotionY);
                ...
                Log.d(TAG, "onTouchEvent: movement==> " + movemment + ", distance===>" + distance + ", delta==> " + delta);
